...
static char lastAnnounceValue[180];

static void (*announceFunc) (char * ) = NULL;

/* ----- production code ------ */
void initGame (void (*passedannounceSentence)(char *)){
	nextIndex=0;
memset(rolls,0,sizeof(rolls));//avoid segmentation errors
	announceFunc=passedannounceSentence;
}
...
int getScore (void){
	...
	sprintf(scoreAnnouncement,"Score is %d",total);
	if (announceFunc!=NULL)
	{
		announceFunc(scoreAnnouncement);
	}
...
void recorderAnnounceer(char * sentence)
{
	strcpy(lastAnnounceValue,sentence);
}
void announce_is_always_called_with_score(void) {

	//given a new game
	initGame(recorderAnnounceer);
	roll(10);
	roll(2);
	roll(5);
	
	getScore();
	TEST_ASSERT_EQUAL_STRING("Score is 24",lastAnnounceValue);
...
int main(void) {
	...
	RUN_TEST(announce_is_always_called_with_score);
    // more RUN_TEST(*) lines....
    // more RUN_TEST(*) lines....
